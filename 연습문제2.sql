...
SELECT FIRST_NAME,SALARY, JOB_ID FROM EMPLOYEES WHERE  LOWER (JOB_ID) = 'it_prog';
--조건 2) 이름은 앞 3문자까지 출력하고 나머지는 *로 출력합니다. 
SELECT (RPAD (SUBSTR(FIRST_NAME, 1,3), LENGTH(FIRST_NAME), '*')) AS NAME, SALARY, JOB_ID FROM EMPLOYEES WHERE LOWER (JOB_ID) = 'it_prog';
--이 열의 열 별칭은 name입니다.(힌트 : rpad와 substr 또는 substr 그리고 length 이용)
--조건 3) 급여는 전체 10자리로 출력하되 나머지 자리는 *로 출력합니다. 
SELECT (RPAD (SUBSTR(FIRST_NAME, 1,3), 10, '*')) AS NAME, RPAD(SALARY,10,'*'), JOB_ID FROM EMPLOYEES WHERE LOWER (JOB_ID) = 'it_prog';
