...
SELECT FIRST_NAME AS 이름 , SALARY AS 급여, SALARY+300 AS "인상된 급여" FROM EMPLOYEES;
...
SELECT FIRST_NAME, HIRE_DATE, SALARY FROM EMPLOYEES  WHERE HIRE_DATE BETWEEN 030101 AND 031231;
...
SELECT FIRST_NAME, SALARY, DEPARTMENT_ID  FROM EMPLOYEES WHERE DEPARTMENT_ID IN 60 ORDER BY SALARY DESC;
...
SELECT FIRST_NAME || ' ' || LAST_NAME || '사원의 급여는 $' || SALARY || '입니다' AS 급여 FROM EMPLOYEES  WHERE FIRST_NAME IN 'Steven' AND LAST_NAME IN 'King';
